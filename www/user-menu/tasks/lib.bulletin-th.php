	...
	class Bulletin_TH {
		...
		public function Create() {
			...
			*/
			...
			$this->pdf->SetFont('AngsanaNew', '', 15);
			$ck_time_create = explode("/", $this->h_date);
			$create_time = mktime(0, 0, 0, $ck_time_create[0], $ck_time_create[1], $ck_time_create[2]); 
			$date = $this->pdf->thai_date($create_time);
			...
			$this->pdf->SetFont('AngsanaNew', '', 15);
			$txt_8 = $this->pdf->conv("         ศูนย์กลางที่  ละติจูด   ".(round($this->eq_latitude, 2) < 0 ? round($this->eq_latitude, 2)*(-1) : round($this->eq_latitude, 2))."   ".(round($this->eq_latitude, 2) > 0 ? "องศาเหนือ" : "องศาใต้")."   ลองจิจูด   ".(round($this->eq_longitude, 2) < 0 ? round($this->eq_longitude, 2)*(-1) : round($this->eq_longitude, 2)).(round($this->eq_longitude, 2) > 0 ? "   องศาตะวันออก   " : "   องศาตะวันตก   "));
			$this->pdf->MultiCell(0, 7, $txt_8, 0, 'J');
			$this->pdf->Ln(5);
