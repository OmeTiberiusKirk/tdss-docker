...
$(document).ready(function(){
  ...
  $(document).ready(function(){
    $("#select-case2").change(function(){
      var case_pagination_url = $('option:selected', this).attr('data-url2');
      window.location.replace(case_pagination_url);
    ...
    if (is_resource($connection)) {
        ...
        if (mysql_num_rows($result) == 1) {
            ...
            if (mysql_num_rows($result) == 1) {
                ...
                //echo("! [notice] not distination dir.\n!Waiting to delete profile ...");
...
$_REQUEST['sort'] = !isset($_REQUEST['sort']) ? "create_date" : $_REQUEST['sort'];
$_REQUEST['list'] = !isset($_REQUEST['list']) ? "ASC" : $_REQUEST['list'];
...
for ($i = 0; $i < $no_page; $i++) {
    $str_jump_page[($i + 1)] =  $_SERVER['PHP_SELF'] . "?section=3&grp_id=" . $_REQUEST['grp_id'] . "&btn=next&start=" . ($i * $limit) . "&limit=" . $limit . "&curr_page=" . ($i + 1) . "&sort=" . $_REQUEST['sort'] . "&list=" . $_REQUEST['list'] ;
}
...
<DIV id=main>
  ...
  <DIV id=middle_column>
    <DIV class="section bordered">
      <H2 id=icon_pick><img src="../../image/color_theory_small_icon.jpg" width="14" height="14">&nbsp;<!-- InstanceBeginEditable name="title_workspace" --><span class="dq_closestoday"> </span>&nbsp;Simulation and Visualization Result<!-- InstanceEndEditable --> </H2>
...
if (mysql_num_rows($rg) > 0) {
    while ($obj = mysql_fetch_object($rg)) { */
        ?>
            <option value="<?php echo $obj->id; ?>" style="font-size:13px; text-align:left" <?php echo $selected_group[$obj->id]; ?>>&nbsp;-
            <?php echo $obj->g_name; ?>
            &nbsp;
            <?php echo $obj->default == "yes" ? "(default)" : ""; ?>
            </option>
            <?php
/*}
} else {*/
...
?>
          ...
          | <a href="import_sim_result.php?section=3&grp_id=<?php echo $_REQUEST['grp_id']; ?>"><font color="#000099">Import simulation result</font></a> |
          ...
          <!--<a href="javascript: void(0)" onClick="javascript: document.location.href='<?php echo $_SERVER['PHP_SELF']; ?>?section=3&grp_id=<?php echo $_REQUEST['grp_id']; ?>';"><font color="#000099">Reload</font></a></div>-->
...
?>
          <table width="10" height="2" border="0">
            <tr>
              <td><?php //echo "<pre>".$sq."</pre>"; ?><table width="0" border="0" cellspacing="2" cellpadding="2">
                  <tr>
...
?>
                      <select name="select_sort_by" id="select_sort_by" onChange="javascript: select_jump(document.getElementById('select_sort_by').value);" style="font-size:13px; text-align:left">
                        <option value="name" <?=$sel['name']?> style="font-size:13px; text-align:left">Case ID</option>
                    ...
                    <td><a href="<?=$_SERVER['PHP_SELF']?>?section=<?=$_REQUEST['section']?>&grp_id=<?=$_REQUEST['grp_id']?>&sort=<?=$_REQUEST['sort']?>&list=ASC">Ascending</a>&nbsp;|</td>
                    <td><a href="<?=$_SERVER['PHP_SELF']?>?section=<?=$_REQUEST['section']?>&grp_id=<?=$_REQUEST['grp_id']?>&sort=<?=$_REQUEST['sort']?>&list=DESC">Descending</a></td>
                  </tr>
                </table></td>
            </tr>
          </table>
...
if (count($sim_result['id']) > 0) {
    ?>
          ...
          <div align="left">
            ...
            &nbsp;Page number :
            <select name="select" id="select-case" style="font-size:13px; text-align:left">
              <?php
for ($p = 1; $p <= $no_page; $p++) {
        ?>
              <option value="<?=$p?>" data-url=" <?=$str_jump_page[$p]?>" <?=$selected_page[$p - 1]?> style="font-size:13px; text-align:left">
              <?=$p?>
              </option>
              <?php
}
...
$today = date('d', time());
    ...
    ?>
                    ...
                    <td align="left" valign="top">Case ID: <strong>
                      <?=$sim_result['name'][$i]?>
              ...
              <td><div id="s<?=$sim_result['id'][$i]?>" style="display:none">
                  <div align="right"><!--<a href="javascript: void(0);" onClick="javascript: enable_map(<?=$sim_result['decimal_lat'][$i]?>, <?=$sim_result['decimal_long'][$i]?>);">View&nbsp;location</a>&nbsp;|&nbsp;--><a href="../bulletin/general.php?id=<?=$sim_result['id'][$i]?>&job_profile_id=<?=$sim_result['job_profile_id'][$i]?>&grp_id=<?=$sim_result['grp_id'][$i]?>" class="style39">Detail</a>&nbsp;<!--|&nbsp;<a href="bulletin.php?section=4.1&id=<?=$sim_result['id'][$i]?>&grp_id=<?=$sim_result['grp_id'][$i]?>">Create&nbsp;Bulletin</a>&nbsp;--><?php if ($_REQUEST['admin'] == "true") {?>|&nbsp;<a href="javascript: void(0);" onClick="javascript: del_item(<?=$sim_result['id'][$i]?>, '<?=$sim_result['name'][$i]?>', <?=$sim_result['grp_id'][$i]?>);">Delete</a>&nbsp;<?php }?></div>
                </div></td>
            </tr>
          </table>
          <table width="10" height="2" border="0">
...
if (count($sim_result['id']) != 0) {
    ?>
          ...
          <div align="left">
            ...
            <input type="button" value="&lt;" onClick="javascript: <?=$str_back?>">
            &nbsp;Page number :
            <select name="select" id="select-case2" style="font-size:13px; text-align:left">
              <?php
for ($p = 1; $p <= $no_page; $p++) {
        ?>
              <option value="<?=$p?>" data-url2=" <?=$str_jump_page[$p]?>" <?=$selected_page[$p - 1]?> style="font-size:13px; text-align:left">
              <?=$p?>
              </option>
              <?php
}
