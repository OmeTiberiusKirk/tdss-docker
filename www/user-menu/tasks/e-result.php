...
<script language="javascript">
	...
	function MM_jumpMenu(targ,selObj,restore){ //v3.0
	  eval(targ+".location='<?=$_SERVER['PHP_SELF']?>?section=<?=$_REQUEST['section']?>&sort="+selObj.options[selObj.selectedIndex].value+"'");
		...
		if(is_resource($connection)) {
			...
			if(mysql_num_rows($result) == 1) {
				...
				if(mysql_num_rows($result) == 1) {
					...
					$sql = "DELETE FROM `sim_result`, `sim_profile`, `sim_point_val`, `job_profile` ";
					$sql .= "USING `sim_result`, `sim_profile`, `sim_point_val`, `job_profile` ";
					...
					$sql .= "`sim_result`.`id` = ".$sim_result_id." AND ";
			...
			sleep(0.5);
			echo "<script language=javascript>document.location.href='".$_SERVER['HTTP_REFERER']."&res=success';</script>";						
		}else die("! no db link");
	}
...
<DIV id=main>
  ...
  <DIV id=left_column>
    ...
    <center>
      <table>
        <tr>
          <td><div align="center">&nbsp;<img src="../../logo.jpg" width="100" height="111" style="border:3px" usemap="#Map"></div></td>
        </tr>
        <tr>
  ...
  <DIV id=middle_column>
    <DIV class="section bordered">
      <H2 id=icon_pick><img src="../../image/color_theory_small_icon.jpg" width="14" height="14">&nbsp;<!-- InstanceBeginEditable name="title_workspace" --><span class="dq_closestoday"> </span>&nbsp;Visualization Result<!-- InstanceEndEditable --> </H2>
      <DIV class=tutorials_list><!-- InstanceBeginEditable name="visualization_output" -->
        <div style="background-color:#D5EBFD; padding:4px 4px 4px 4px; border-top:#999999 1px solid"><strong>Operations&nbsp;: &nbsp;</strong><a href="javascript: void(0)"><font color="#000099">Select  group</font></a>
          ...
          <a href="<?=$_SERVER['PHP_SELF']?>?section=3"><font color="#000099">Reload</font></a></div>
	...
	?>
                      <select name="jumpMenu" id="jumpMenu" onChange="MM_jumpMenu('parent',this,0)" style="font-size:13px; text-align:left">
                    ...
                    <td><a href="<?=$_SERVER['PHP_SELF']?>?section=<?=$_REQUEST['section']?>&sort=<?=$_REQUEST['sort']?>&list=ASC">[A&nbsp;to&nbsp;Z]</a></td>
                    <td><a href="<?=$_SERVER['PHP_SELF']?>?section=<?=$_REQUEST['section']?>&sort=<?=$_REQUEST['sort']?>&list=DESC">[Z&nbsp;to&nbsp;A]</a></td>
                  </tr>
                </table></td>
            </tr>
          </table>
          ...
          <table width="100%" border="0" cellpadding="0" cellspacing="0">
            <tr onMouseOver="javascript: this.style.background='#D0EFFB'; this.style.borderBottom='#A4ED8B 1px solid'; this.style.borderTop='#A4ED8B 1px solid';  document.getElementById('s<?=$sim_result['id'][$i]?>').style.display='';" onMouseOut="javascript: this.style.background=''; this.style.borderBottom=''; this.style.borderTop=''; document.getElementById('s<?=$sim_result['id'][$i]?>').style.display='none';" title="<?="Create date : ".date('h:i:s A d/m/Y', $sim_result['datetime'][$i])?>">
              <td ><table border="0" cellpadding="0" cellspacing="1">
                  <tr>
                    <td rowspan="6" valign="top"><a href="../bulletin/general.php?id=<?=$sim_result['id'][$i]?>&job_profile_id=<?=$sim_result['job_profile_id'][$i]?>&grp_id=<?=$sim_result['grp_id'][$i]?>">
                      <?php
                    $today = date('d', time());
					$cd = date('d', $sim_result['datetime'][$i]);
					#if($today == $cd) {
					if($sim_result['magnitude'][$i] >= 8.0) {
					?>
              ...
              <td><div id="s<?=$sim_result['id'][$i]?>" style="display:none">
                  <div align="right"><a href="javascript: void(0);" onClick="javascript: enable_map(<?=$sim_result['decimal_lat'][$i]?>, <?=$sim_result['decimal_long'][$i]?>);">View&nbsp;location</a>&nbsp;|&nbsp;<a href="javascript: void(0);" onClick="javascript: del_item(<?=$sim_result['id'][$i]?>, '<?=$sim_result['name'][$i]?>', <?=$sim_result['grp_id'][$i]?>);">Delete</a>&nbsp;|&nbsp;<a href="bulletin.php?section=4.1&id=<?=$sim_result['id'][$i]?>&grp_id=<?=$sim_result['grp_id'][$i]?>">Create&nbsp;Bulletin</a>&nbsp;|&nbsp;<a href="../bulletin/general.php?id=<?=$sim_result['id'][$i]?>&job_profile_id=<?=$sim_result['job_profile_id'][$i]?>&grp_id=<?=$sim_result['grp_id'][$i]?>" class="style39">More...</a></div>
                </div></td>
            </tr>
          </table>
          <table width="10" height="2" border="0">
...
<map name="Map">
  <area shape="rect" coords="5,3,96,110" href="http://www.thaigrid.or.th" title="Thai National Grid Center">
</map>
<map name="Map2">
  <area shape="rect" coords="5,4,28,45" href="http://www.chula.ac.th" title="Chulalongkorn University">
</map>
</BODY>
<!-- InstanceEnd -->
</HTML>
