	...
	$h_time = date('H:i:s', $time_now);
		...
		$sql_eta = "SELECT * FROM `sim_point_val`, `observe_point` 
				WHERE 
					...
					`sim_point_val`.`type` LIKE 'ETA' AND `sim_point_val`.`region_no` = 2 AND
					`sim_point_val`.`sim_result_id` = ".$_REQUEST['id'];
					
		$sql_zmax = "SELECT * FROM `sim_point_val`, `observe_point` 
				WHERE 
					`sim_point_val`.`id_point` = `observe_point`.`observ_point_id` AND 
					`sim_point_val`.`type` LIKE 'ZMAX' AND `sim_point_val`.`region_no` = 2 AND
					`sim_point_val`.`sim_result_id` = ".$_REQUEST['id'];
    ...
    <DIV class="section bordered">
      ...
      <DIV class=tutorials_list><!-- InstanceBeginEditable name="visualization_output" -->
        <div style="background-color:#D5EBFD; padding:4px 4px 4px 4px; border-top:#999999 1px solid"><strong>Operations&nbsp;:&nbsp;</strong><font color="#000099"> Select group </font><select name="grp" id="grp" onChange="jump(this.value)" style="font-size:13px; text-align:left">
		  ...
		  ?>
          </select> | <a href="result.php?section=3"><font color="#000099">Create new </font></a> | <a href="bulletin_release.php?section=<?=$_REQUEST['section']?>"><font color="#000099">List of released bulletin</font></a> | <a href="<?=$_SERVER['PHP_SELF']?>?section=<?=$_REQUEST['section']?><?=strlen($_REQUEST['id']) > 0 ? "&id=".$_REQUEST['id'] : ""?>"><font color="#000099">Reload</font></a></div>
        ...
        <form action="bulletine_create_script.php?grp_id=<?=$_REQUEST['grp_id']?>" method="post">
              ...
              <td valign="top"><table width="443" border="0" class="tbl_border" cellpadding="4" cellspacing="0" id="search_tbl3" style="display:">
                <tr bgcolor="#FFFFFF">
                  <td colspan="3" valign="top" bordercolor="#999999" bgcolor="#7B869A" ><div align="center" class="style31">
                      <div align="left">Bulletin information of <u><?=$g_name?></u><input type="hidden" name="h_gname" value="<?=$g_name?>"><input type="hidden" name="h_gid" value="<?=$g_id?>"></div>
                  </div></td>
                </tr>
          ...
          <table border="0" cellpadding="4" cellspacing="0" id="search_tbl2" style="display:" class="tbl_border">
            ...
            <tr onMouseOver="javascript: this.style.background='#D0EFFB';" onMouseOut="javascript: this.style.background='';">
              ...
              <div align="left"><?=$obj->province_t?><input type="hidden" name="ob[province][<?=$i?>]" value="<?=$obj->province_t?>"><input type="hidden" name="ob[province_eng][<?=$i?>]" value="<?=$obj->province?>"></div></th>
              <th scope="col" valign="middle" class="tbl_border_right"><div align="left"><?=$obj->name_t?><input type="hidden" name="ob[area][<?=$i?>]" value="<?=$obj->name_t?>"><input type="hidden" name="ob[area_eng][<?=$i?>]" value="<?=$obj->name?>"></div></th>
              <th scope="col" valign="middle"><img src="../../image/buttonorange.gif" alt="0" width="7" height="10"></th>
