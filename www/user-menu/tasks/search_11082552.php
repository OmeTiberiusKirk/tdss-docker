  ...
  <DIV id=middle_column>
    <DIV class="section bordered">
      ...
      <DIV class=tutorials_list><!-- InstanceBeginEditable name="visualization_output" -->
          ...
          <table border="0" cellpadding="4" cellspacing="0" id="search_tbl" style="display:">
            ...
            <tr>
              ...
              <th align="left" bgcolor="#F0F0F0" scope="col">&nbsp;
				...
				if(strlen($_POST['date']) == 0 && !isset($_REQUEST['from'])) {
					$_SESSION['date'] = $date;
				}
				
				if(strlen($_POST['time']) == 0 && !isset($_REQUEST['from'])) {
					$_SESSION['time'] = $time;
				}
			  ?></th>
            ...
            <tr>
              ...
              <th colspan="2" align="left" bgcolor="#F0F0F0" scope="col"><font color="#990000" style="font-weight:normal"><?=isset($_REQUEST['from']) ? $_SESSION['location'] : ""?></font></th>
            </tr>
            <tr>
              <th align="left" valign="middle" bgcolor="#F0F0F0" scope="col">&nbsp;</th>
              <th align="left" valign="middle" bgcolor="#F0F0F0" scope="col">&nbsp;</th>
            ...
            <tr>
              ...
              <th scope="col" valign="middern"><img src="../../image/buttonorange.gif" alt="0" width="7" height="10"></th>
              <th align="left" scope="col"><input name="magnitude" type="text" id="magnitude" style="width:100px; text-align:center; font-weight:bold" value="<?=$_SESSION['magnitude']?>"></th>
              <th align="left" scope="col">&nbsp;</th>
            </tr>
            <tr>
              <th align="left" valign="middle" bgcolor="#F0F0F0" scope="col">&nbsp;</th>
        ...
        <div id="search_result_box"style="background-color:#FFFFCC; padding:4px 4px 4px 4px; border-top:#999999 1px solid"><strong>
          <?php
				require_once('../../library/connectdb.inc.php');
					...
					$long = (float)($_SESSION['long']);

					$sq = "SELECT `id`, `job_profile_id`, `name`, `magnitude`, `depth`, `decimal_lat`, `decimal_long`, ROUND(SQRT(POW(`decimal_lat` - (".floatval($lat)."), 2)+POW(`decimal_long` - (".floatval($long)."), 2)), 3) AS `R` FROM `sim_result` WHERE ";
					...
					if($_REQUEST['grp_id'] == 1) {
						if((float)($_SESSION['magnitude']) > 0 ) {
							if((float)$_SESSION['magnitude'] < 7.5)
								$c = "`magnitude` = 7.5 ";
							elseif ((float)$_SESSION['magnitude'] >= 7.5 && (float)$_SESSION['magnitude'] < 8.0) {
								$c = "`magnitude` = 8.0 ";
							}elseif((float)$_SESSION['magnitude'] >= 8.0 && (float)$_SESSION['magnitude'] < 8.5) {
								$c = "`magnitude` = 8.5 ";
							}elseif((float)$_SESSION['magnitude'] >= 8.5) {
								$c = "`magnitude` = 9.0 ";
							}
							$sq_c[] = $c;
						}
						
						if((float)$_SESSION['depth'] > 0) {						
							if((float)($_SESSION['depth']) <= 10 || (float)($_SESSION['depth']) < 20) {
								$sq_c[] = " `depth` = 10 ";
							}elseif((float)$_SESSION['depth'] >= 20) {
								$sq_c[] = " `depth` = 30 ";
							}
						}
					}else {
						if((float)$_SESSION['magnitude'] <= 8.0 && (float)$_SESSION['magnitude'] > 0) {
							$c = "`magnitude` = 8.0 ";
						}elseif((float)$_SESSION['magnitude'] > 8.0 && (float)$_SESSION['magnitude'] <= 8.5) {
							$c = "`magnitude` = 8.5 ";
						}elseif((float)$_SESSION['magnitude'] > 8.5) {
					...
					#$c = " MIN(SQRT(EXP(`decimal_lat` - ".$lat.")+EXP(`decimal_long` - ".$long.")))) ";
					/*
					SELECT 
						`id`, `name`, ROUND(SQRT(POW(`decimal_lat` - 0, 2)+POW(`decimal_long` - 0, 2)), 3) AS `R` 
					FROM 
						`sim_result` 
					WHERE 
						uid=3 AND `magnitude` = 7.5 AND `depth` = 10 AND `grp_id` = 1
					*/
					#$sq_c[] = $c;
					/*
					...
					*/
					...
					$sq .= implode(" AND ", $sq_c)." AND `grp_id` = ".$_REQUEST['grp_id']." ORDER BY `id` DESC";
		...
		?>
          </strong><table width="10" height="1" border="0">
            <tr>
              <td></td>
            </tr>
          </table> <?php if($num_rows != 0){?>
          <div align="left">
              ...
              <tr <?php if($g != 0 || $page_start != 0) {?> onMouseOver="javascript: this.style.background='#D0EFFB'; this.style.borderBottom='#A4ED8B 1px solid'; this.style.borderTop='#A4ED8B 1px solid';  document.getElementById('s<?=$sim_result[$g]['id']?>').style.display='';" onMouseOut="javascript: this.style.background=''; this.style.borderBottom=''; this.style.borderTop=''; document.getElementById('s<?=$sim_result[$g]['id']?>').style.display='none';"<?php } ?> >
                    ...
                    <tr>
					  ...
					  ?>
                        ...
                        <table width="0" border="0" cellspacing="1" cellpadding="0">
                          <tr>
                            <td>Magnitude: <font color="#0000CC">
                              <?=number_format($sim_result[$g]['magnitude'], 1)?></font>
                              ,                           Depth: <font color="#0000CC">
                              <?=$sim_result[$g]['depth']?>
                              </font>km.</td>
                          </tr>
                          <tr>
				...
				?>
                  <div id="s<?=$sim_result[$g]['id']?>" style="display:<?=$display?>">
                    ...
                    <div align="right"><a onClick="javascript: create_bulletin(<?=$sim_result[$g]['id']?>, '<?=$sim_result[$g]['name']?>');" href="javascript: void(0);">Create&nbsp;Bulletin</a> | Reference ID =
                      <?=$sim_result[$g]['name']?>
