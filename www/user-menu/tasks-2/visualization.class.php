<?php
	...
	class Visualization {
		...
		public function __construct($type, $style, $working_dir, $region, $grp_id) {
			global $template;
			global $visualizer;
			global $visout;
			...
			switch($type) {
				...
				case "Height" :
					if($style == "3D")
						$this->template_net_loc = $template['G'.$grp_id]['height']['3d'][''.$region.''];
					else
						$this->template_net_loc = $template['G'.$grp_id]['height']['2d'][''.$region.''];
					break;
					
			    case "The Elapsed Time Arrival" :
			
					if($style == "3D")
						$this->template_net_loc = $template['G'.$grp_id]['arraival-time']['3d'][''.$region.''];
					else
						$this->template_net_loc = $template['G'.$grp_id]['arraival-time']['2d'][''.$region.''];															               
			     	break;
		...
		public function createNetworkFile($url_ds_region, $url_ds_data, $tmp_filename, $zoom_t, $resolution_t, $rotate_x_t, $rotate_y_t, $rotate_z_t, $gridsize_x_t, $gridsize_y_t, $timestep_t, $region_t, $data_t, $axes_y_position, $axes_y_label, $axes_x_position, $axes_x_label, $point_name, $point_position, $contour_line, $contour_line2, $axes_scale, $network_type, $reg_no) {
			global $tmpfile_prefix;
			
			$region_import_url = $url_ds_region;
			$uvm_import_url = $url_ds_data;
			$_SESSION['dir_image'][] = "image_".$network_type."_".$reg_no;
			$image = $this->remote_working_dir."image_".$network_type."_".$reg_no."/".$this->visualization_output_image_name;
			$colorbar = $this->remote_working_dir."image_".$network_type."_".$reg_no."/".$this->visualization_output_colorbar_name;
			$zoom = $zoom_t;
			...
			if ($handle) {
				...
				while (!feof($handle))  {
					...
					if(preg_match("/__CONTOUR_LINE2__/", $buffer))
						$buffer = str_replace("__CONTOUR_LINE2__", $contour_line2, $buffer);
						
					if(preg_match("/__LABEL_SCALE__/", $buffer))
						$buffer = str_replace("__LABEL_SCALE__", $axes_scale, $buffer);					
		...
		public function calculate_Lat_Long($Lat_min, $Lipda_min, $Philipda_min, $Lat_max, $Lipda_max, $Philipda_max, $Scale_Lipda, $Scale_Philipda, $Grid_x, $Grid_y, $status) {
			global $value;
			$value = array();
					...
					if($status == "lat"){
					    ...
					    if($Lat_min >= 0){
							...
							if($Philipda_min == 0){
								...
								//$Long_1 = 95;  $Long_Lipda_1 = 59; $Long_Philipda_1 = 45;
								//$Long_2 = 99; $Long_Lipda_2 = 0; $Long_Philipda_2 = 0; 
				...
				if($find_max == 1){
					if($gap >= 1){
									...
									if($min_clear_flot[$h+1] == 0){
										if($min_clear_flot[$h+2] == 0){
											if($min_clear_flot[$h+3] == 0){
												...
												if($min_without_float[$h-1] == 10){
													$min_without_float[$h-1] = 0;
													$min_without_float[$h-2] = $min_without_float[$h-2] + 1;
													if($min_without_float[$h-2] == 10){
														$min_without_float[$h-2] = 0;
														$min_without_float[$h-3] = $min_without_float[$h-3] + 1;
													}
												}
												$check_exist_float = 0;}
											}else{ 
						...
						if($res == 0 && (($gap != 2) or ($gap != 5))){
							...
							// 674 edit for 99.03125
							$b = 0;
							foreach ($min_without_float as $indexs => $values) {
								 $arrs[$b]['index'] = $indexs;
								 $arrs[$b]['val'] = $values;
								 $b++;
							}
								...
								while($flag_float == 0){
									if($min_clear_flot[$r] == "." || $min_clear_flot[$r] == NULL){
										if($min_clear_flot[$r+1] >= 5){
											...
											if($min_without_flot[$r-1] > 9){
												$min_without_float[$r-2] = $min_clear_flot[$r-2];// + 1;
												if($min_clear_flot[$r-3] > 9){
													$min_without_float[$r-3] = $min_clear_flot[$r-3];// + 1;
												}
											}
										}
										$flag_float = 1;
		...
		public function Calculate_AutoAxes2( $axes_min, $axes_max, $gridsize) {
			global $axes;
			$axes = array();
			$max = $axes_max;
			$min = $axes_min;
		...
		public function Calculate_Value_4_Point($point) {   
			...
			for($j=0; $j<count($point); $j++) {
				$value['name'][$j] = $point[$j]['name'];
				$lat_1[$j] = $point[$j]['lat_degree'];
				$lat_2[$j] = $point[$j]['lat_lipda'];
				$lat_3[$j] = $point[$j]['lat_philipda'];
				$long_1[$j] = $point[$j]['long_degree'];
				$long_2[$j] = $point[$j]['long_lipda'];
				$long_3[$j] = $point[$j]['long_philipda'];
		...
		public function Calculate_Point_Table($point_data, $point_name, $Grid_x, $Grid_y, $OUT_filename) {
			global $point_table;
			$point_table = array();
			...
			for($i = 0; $i <= count($point); $i++) {
				if($point[round($point_data[$i]['long'])][round($point_data[$i]['lat'])] != NULL){
					$point_table[$point_name[$i]] = $point[round($point_data[$i]['long'])][round($point_data[$i]['lat'])];
				}
			}
			return $point_table;
		}
