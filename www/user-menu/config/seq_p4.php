...
<?php
		...
		$data = array();
		if($_REQUEST['id']) {
			$var_id = base64_decode($_REQUEST['id']);
			require_once('../../library/connectdb.inc.php');
			$sq = "SELECT * FROM seq_config_param  WHERE  var_id=".$var_id." ;";
			...
			if($res) {	
				if(mysql_num_rows($res) == 1 ) {									
					...
					$sim['id'] = $obj->var_id;
					$sim['data'] = $obj->data;
					$sim['description'] = $obj->description;
				}
				else
					$no = 0;
			}
  ...
  <DIV id=middle_column>
    <DIV class="section bordered">
      ...
      <DIV class=tutorials_list><!-- InstanceBeginEditable name="visualization_output" -->
        ...
        <form action="handler.php" method="post" id="fval">
          ...
          <table border="0" cellspacing="0" cellpadding="3">
            <tr>
              <td valign="middle" bgcolor="#F0F0F0">Template name </td>
              <td valign="middle"><img src="../../image/buttonorange.gif" alt="0" width="7" height="10"></td>
              <td><input name="conf_name[name]" type="text" id="conf_name[name]" value="<?=$sim['name']?>" style="width:350px"></td>
              <td>&nbsp;</td>
            </tr>
            <tr>
              <td valign="top" bgcolor="#F0F0F0">Description </td>
              <td valign="top"><img src="../../image/buttonorange.gif" alt="0" width="7" height="10"></td>
              <td><textarea name="conf_name[des]" rows="3" id="description" style="width:350px"><?=$sim['description']?>
</textarea></td>
              <td>&nbsp;</td>
            </tr>
          </table>
          <fieldset style="background-image:url(../../image/gradient-inner.png); background-repeat:repeat-x; background-color:#FFFFFF">
          <legend class="dq_closestoday"><strong> <font color="#990000">Global Parameter</font> </strong></legend>
          <?php 	
					#echo "<pre>";
					$data = explode("\n", stripslashes($sim['data']));
          ...
          <table border="0" cellpadding="3" cellspacing="0" bgcolor="#FFFFFF" style="background-image:url(../../../image/gradient-inner.png); background-repeat:repeat-x">
            ...
            <tr>
              <td valign="middle" bgcolor="#F7F8DC" class=""><?=$i++?>
                . </td>
              <td valign="middle" bgcolor="#F7F8DC" class=""><input name="des[global][<?=$var_name?>]" value="<?=$var_info?>" type="text" class="text_input_var" size="60"/></td>
          ...
          <table border="0" cellpadding="3" cellspacing="0" bgcolor="#FFFFFF" style="background-image:url(../../../image/gradient-inner.png); background-repeat:repeat-x">
            ...
            <tr>
              <td valign="middle" bgcolor="#F7F8DC" class=""><?=$i++?>
                .</td>
              <td valign="middle" bgcolor="#F7F8DC" class=""><input name="des[input][<?=$var_name?>]" value="<?=$var_info?>" type="text" class="text_input_var" size="60"/></td>
              <td valign="middle" bgcolor="#F7F8DC" class=""><font color="#000066">
                <?=$var_name?>
                </font>=</td>
              <td valign="middle" bgcolor="#F7F8DC" class=""><table border="0" cellspacing="0" cellpadding="2">
                  <tr>
                    <td><input name="val[input][<?=$var_name?>]" type="text" class="text_input_var" value="<?=$data['val']['input'][$var_name]?>" size="20" id="val[input][<?=$var_name?>]" /></td>
          ...
          <table border="0" cellpadding="3" cellspacing="0" bgcolor="#FFFFFF" style="background-image:url(../../../image/gradient-inner.png); background-repeat:repeat-x">
            ...
            <tr>
              <td valign="middle" bgcolor="#F7F8DC" class=""><?=$i++?>
                .</td>
              <td valign="middle" bgcolor="#F7F8DC" class=""><input name="des[output][<?=$var_name?>]" value="<?=$var_info?>" type="text" class="text_input_var" size="60"/></td>
              <td valign="middle" bgcolor="#F7F8DC" class=""><font color="#000066">
                <?=$var_name?>
                </font>=</td>
