...
<?php
	...
	if(!isset($_SESSION['template'])) {
		session_unregister("template");	
		foreach($_POST['preset_id'] as $template_id => $id) {
			$label = spliti("_", $id);
			if(count($label) == 2) {
				if($label[0] == "d") {
					$sql = "SELECT * FROM `seq_config_param`  WHERE  `var_id`= ".$label[1].";";
					$default = "yes";
				}
			}else {
				$label[1] = $id;
				$sql = "SELECT `name`, `param_contents` AS `data`, `id` FROM `sim_profile`  WHERE  `id` = ".$label[1].";";
			}
		}	
		...
		if($res) {	
			if(mysql_num_rows($res) == 1 ) {
				...
				$_SESSION['template']['name'] = $obj->name;
				$_SESSION['template']['data'] = $obj->data;
				$_SESSION['template']['unserialized_data'] = $obj->data;
				$_SESSION['template']['id'] = ($default == "yes") ? $obj->var_id : $obj->id;
			}else
				$no = 0;
		}
	}	
    ...
    <DIV class="section bordered">
      ...
      <DIV class=tutorials_list><!-- InstanceBeginEditable name="visualization_output" -->
        ...
        <div style="background-color:#D7F1FB; padding:4px 4px 4px 4px; border-top:#0099FF 1px solid;">
          <div align="center"><strong>Step 3 of 6</strong><font color="#000099"></font></div>
        </div>
        ...
        <form action="../sequential/param_2.php?section=1" method="post" >
          <fieldset style="background-image:url(../../../image/gradient-inner.png); background-repeat:repeat-x; background-color:#FFFFFF">
          <legend class="dq_closestoday"><strong> <font color="#990000">Specify global parameter</font> </strong></legend>
          <?php 	
					#echo "<pre>";
					...
					$_SESSION['template']['data'] = $temp;
					$data = isset($_SESSION['data']) ? $_SESSION['data'] : $temp;
					#echo "</pre>"; 
					#exit;
          ...
          <table border="0" cellpadding="3" cellspacing="0" bgcolor="#FFFFFF" style="background-image:url(../../../image/gradient-inner.png); background-repeat:repeat-x">
            ...
            <?php 
			$i=1;
			foreach($data['des']['global'] as $var_name => $var_info){
				$var_info = stripslashes($var_info);
			?>
            <tr>
              <td valign="middle" bgcolor="#F7F8DC" class=""><?=$i++?>
                . </td>
              <td valign="middle" bgcolor="#F7F8DC" class=""><input name="des[global][<?=$var_name?>]" value="<?=$var_info?>" type="text" class="text_input_var" size="60" onMouseOver="Tip('<?=addslashes($var_info)?>', WIDTH, 400, TITLE, '<b>Variable name:</b> <?=$var_name?>, Value: <?=(strlen($data['val']['global'][$var_name] < 1) ? "(not defined)": $data['val']['global'][$var_name])?>', SHADOW, true, FADEIN, 300, FADEOUT, 300, STICKY, 1, CLOSEBTN, true, DELAY, 400)" onMouseOut="UnTip()"/></td>
              <td valign="middle" bgcolor="#F7F8DC" class=""><font color="#000066">
                <?=$var_name?>
                </font>=</td>
              <td valign="middle" bgcolor="#F7F8DC" class=""><input name="val[global][<?=$var_name?>]" type="text" class="text_input_var" value="<?= $data['val']['global'][$var_name]?>" size="20" />
                </strong></font></td>
            </tr>
            <?php
			}
        ...
        <div id="light" class="white_content">
          <iframe src="../data-source/resource_browser.php?type=sim&data=region" style="border-width:0px;" width="100%" height="100%">Something wrong !</iframe>
        </div>
        <!-- end -->
